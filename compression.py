def main():
    ...
    resultList = createList(otherText)
    resultText = createText(resultList)
    modifiedText = replaceWord(numberDictionary, resultList)
    resoredText = restoreWord(numberDictionary, modifiedText)
    newDictionary = createDictionary(listForArgument)
    bestDictionary = improvedDictionary(dictionaryToImprove)


def createList(string):
    newList = string.split(" ")
    print("creating list: ", newList)
    return newList


def createText(list):
    newText = ' '.join(list)
    print("creating string: ", newText)
    return newText


def replaceWord(dict, list):
    counter = -1
    for item in list:
        counter += 1
        for key, value in dict.items():          
            if item == key: 
                list[counter] = value
    print("replacing word in list: ", list)         
    return list


def restoreWord(dict, list):
    counter = -1
    for item in list:
        counter += 1
        for key, value in dict.items():          
            if item == value: 
                list[counter] = key
    print("restoring word in list: ", list)         
    return list
...
def createDictionary(list):
    ...
    for item in list:
        ...
        for nextItem in list:
            if item == nextItem:
                counter += 1
        newDict.update({item : counter})
    print("created dict ", newDict)
    return newDict


def improvedDictionary(dict):
    newDict = {}
    ...
    for key, value in dict.items():
        ...
        for letter in key:
            counter += 1
        if counter >= 3 and value >= 2:
            newDict.update({key : counter})
        #print(newDict)

    newCounter = 1   
    for key, value in newDict.items(): 
            newDict.update({key : newCounter})
            newCounter += 1
            #print(newDict)
            
    print("improved dict ", newDict)
    return newDict
